...
export function createSeason(
  ...
  for (let i = 0; i < days.length; i++) {
    ...
    for (let j = 0; j < shuffledPlayers.length; j++) {
      ...
      putPlayer(player, day.pods[0], day, days, placementOptions, players);
    }
...
function playerHasBeenInPod(player: string, pod: Pod, days: Day[]): boolean {
  const prevPods = days
    .map((d) => d.pods)
    .flat()
...
function movePlayer(player: string, from: Pod, to: Pod) {
